const MAP_ID = window.DayzMapScriptHelpers.getMapId(window.DayzMapBootstrap?.defaultMapId || 'cherno');
const STORAGE_KEYS = window.DayzMapScriptHelpers.getStorageKeys(MAP_ID);
const MAP_RUNTIME = window.DayzMapRuntime.get(MAP_ID);
const CONFIG = MAP_RUNTIME.config;
const EXPORT_SERVERS = MAP_RUNTIME.exportServers;
...
class DayZMap {
    ...
    leafletToGameCoords(leafletLatLng) {
        return window.DayzMapScriptHelpers.leafletToGameCoords(leafletLatLng, CONFIG);
    }
			...
			if (toolsDropdownBtn && toolsDropdownMenu) {
				...
				if (splitterBtn) {
					splitterBtn.addEventListener('click', () => {
						window.open(window.DayzMapScriptHelpers.getAssetPath('shared/splitter/splitter.html'), '_blank');
						toolsDropdownMenu.classList.remove('show');
						toolsDropdownBtn.classList.remove('active');
					});
				}
				...
				if (timersBtn) {
					timersBtn.addEventListener('click', () => {
						window.open(window.DayzMapScriptHelpers.getAssetPath('shared/timers/timers.html'), '_blank');
						toolsDropdownMenu.classList.remove('show');
						toolsDropdownBtn.classList.remove('active');
					});
				}
				...
				if (downloadLinkBtn) {
					downloadLinkBtn.addEventListener('click', () => {
						...
						link.href = window.DayzMapScriptHelpers.getAssetPath('shared/lnk/LBGroup.rar');
	...
	showCoordsTooltip() {
		...
		tooltip.innerHTML = window.DayzMapScriptHelpers.buildCoordsTooltipHtml(CONFIG);
	...
	renderMarkersList() {
		...
		// Удаляем существующие placeholder перед перерисовкой, если не идет drag
		if (!this.isDragging) {
	...
	saveMarkers() {
		...
		this.saveMarkersTimeout = setTimeout(() => {
			try {
				localStorage.setItem(STORAGE_KEYS.data, JSON.stringify(this.pendingSaveData));
			} catch (e) {
	...
	saveSearchHistory() {
		try {
			localStorage.setItem(STORAGE_KEYS.searchHistory, JSON.stringify(this.searchHistory));
		} catch (e) {
					...
					<div class="server-presets">
						<details open>
							...
							<div class="preset-servers" style="margin-top: 10px;">
								${window.DayzMapScriptHelpers.renderPresetServerRows(EXPORT_SERVERS)}
							</div>
						</details>
					</div>
